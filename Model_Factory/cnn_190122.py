...
def inference_l2reg(images, **kwargs): #batchSize=None, phase='train', outLayer=[13,13], existingParams=[]
    ...
    numParallelModules = kwargs.get('numParallelModules')
    ############# CONV_P0  -  1024 x 64
    ...
    fireOut = tf.split(fireOut, numParallelModules, 3) # split along last dimension to [nt] places
    fireOut[0] = tf.concat([fireOut[0], fireOut[1]], 3)
    numSeqModules = numParallelModules-1
    ...
    ############# CONV_M4  -  64 x 4
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_module_l2regul('conv_p3', fireOut, prevExpandDim,
                                                                  {'cnn1x1': modelShape[4]}, numSeqModules,
                                                                  wd=wd, 
                                                                  stride=[1, 1, 1, 1],
                                                                  **kwargs)
    l2reg += l2regLayer
    ############# CONV_M5  -  64 x 4
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_module_l2regul('conv_m5', fireOut_m4, prevExpandDim_m4,
                                                                  {'cnn1x3': modelShape[5]}, numSeqModules,
                                                                  wd=wd, 
                                                                  stride=[1, 1, 2, 1],
                                                                  **kwargs)
    l2reg += l2regLayer
    ############# CONV_M6  -  32 x 4
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_module_l2regul('conv_m6', fireOut, prevExpandDim,
                                                                  {'cnn1x1': modelShape[6]}, numSeqModules,
                                                                  wd=wd, 
                                                                  stride=[1, 1, 1, 1],
                                                                  **kwargs)
    l2reg += l2regLayer
    ############# CONV_M7  -  16 x 4
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_module_l2regul('conv_m7', fireOut, prevExpandDim,
                                                                  {'cnn1x3': modelShape[7]}, numSeqModules,
                                                                  wd=wd, 
                                                                  stride=[1, 1, 2, 1],
                                                                  **kwargs)
    l2reg += l2regLayer
    ...
    ############# Inception to get FC
    ############# CONV_INC1 - 16 x 4
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_inception_module_l2reg('conv_inc1', fireOut, prevExpandDim,
                                                       {'cnnFC': modelShape[8]}, numSeqModules,
                                                       wd, **kwargs)
    l2reg += l2regLayer
    ############# CONV_M8  -  4 x 2
    fireOut, prevExpandDim, l2regLayer = model_base.conv_fire_parallel_inception_module_l2reg('conv_inc2', fireOut, prevExpandDim,
                                                       {'cnnFC': modelShape[8]}, numSeqModules,
                                                       wd, **kwargs)
    l2reg += l2regLayer
    #######################################
    ############# Linearization assertion  -  [batchsize, 1, 1, prevExpandDim]
    fireOut = tf.reshape(fireOut, [batchSize, prevExpandDim, numSeqModules])
    #######################################
    ############# Dropout before FC
    ...
    ############# FC1 
    fireOut, prevExpandDim, l2regLayer = model_base.fc_fire_parallel_module_l2regul('fc1', fireOut, prevExpandDim,
                                                             {'fc': modelShape[9]}, numSeqModules,
                                                             wd, **kwargs)
    l2reg += l2regLayer
    ############# FC2 - output layer
    fireOut, prevExpandDim, l2regLayer = model_base.fc_fire_parallel_module_l2regul('fc2_output', fireOut, prevExpandDim,
                                                             {'fc': kwargs.get('outputSize')}, numSeqModules,
                                                             wd, **kwargs)
    l2reg += l2regLayer
    pritn(fireOut.get_shape())
    #######################################
